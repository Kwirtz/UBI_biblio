...
oma2 <- c(0,0,0,0)
#df$`minimum income guarantee v2` = df$`minimum income guarantee`+df$`guaranteed minimum income`
df_long <- df %>%
  select(`basic income`, `year`) %>%
  filter(year >= 1950 & year <= 2010) %>%
  pivot_longer(cols = -year, # exclude year column
...
# Apply the moving average to each variable
df_long <- df_long %>%
  group_by(Variable) %>%
  mutate(MA_Value = moving_average(Value))
...
Fig1_a <- ggplot(df_long, aes(x = year, y = MA_Value, color = Variable)) +
